...
- **یکتاسازی مبلغ و انقضای پرداخت به ازای هر سرویس**: از پنل → سرویس‌ها → «تنظیمات مبلغ/انقضا» می‌توانید مشخص کنید که آیا درگاه خودش مبلغ تداخل‌دار را کمی تغییر بدهد تا یکتا شود، یا این مسئولیت با خود سایت فروشنده باشد (و درگاه فقط خطای `amount_conflict` برگرداند)؛ همچنین انقضای پرداخت (بر حسب ساعت) را تنظیم کنید — این مقدار همیشه از پنل خوانده می‌شود و هیچ فیلدی در درخواست API نمی‌تواند آن را override کند (تا اسکریپت‌های فروشنده که مقدار ثابتی می‌فرستند، تنظیمات پنل را بی‌اثر نکنند).
