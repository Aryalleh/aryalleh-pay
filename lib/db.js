...
export async function createService(env, name, description, callbackUrl, autoAdjustAmount = true, defaultExpireHours = 1) {
    const token = generateServiceToken();
    const res = await env.DB.prepare(
        `INSERT INTO services (name, description, token, callback_url, auto_adjust_amount, default_expire_hours)
         VALUES (?,?,?,?,?,?)`
    ).bind(name, description, token, callbackUrl, autoAdjustAmount ? 1 : 0, defaultExpireHours).run();
    return { id: res.meta.last_row_id, token };
}

export async function updateServiceSettings(env, id, autoAdjustAmount, defaultExpireHours) {
    await env.DB.prepare(
        "UPDATE services SET auto_adjust_amount=?, default_expire_hours=? WHERE id=?"
    ).bind(autoAdjustAmount ? 1 : 0, defaultExpireHours, id).run();
}
...
// Same as isAmountPending, but ignores one specific payment row — used when
// changing that row's own amount, so it never conflicts with itself.
export async function isAmountPendingExcluding(env, amountRials, excludePaymentId) {
    const row = await env.DB.prepare(
        `SELECT id FROM pending_payments
         WHERE amount_rials=? AND status='pending' AND id!=?
           AND (expires_at IS NULL OR expires_at > datetime('now'))
         LIMIT 1`
    ).bind(amountRials, excludePaymentId).first();
    return !!row;
}
...
// Same as findUniqueAmount, excluding one payment row from the collision
// check (see isAmountPendingExcluding).
export async function findUniqueAmountExcluding(env, baseAmount, excludePaymentId, maxAttempts = 30) {
    let amount = baseAmount;
    for (let i = 0; i < maxAttempts; i++) {
        if (!(await isAmountPendingExcluding(env, amount, excludePaymentId))) return amount;
        amount = baseAmount + Math.floor(Math.random() * 900) + 100 * (i + 1);
    }
    throw new Error("در حال حاضر امکان یافتن مبلغ یکتا نبود، کمی بعد دوباره تلاش کنید");
}
...
/**
 * Renews an existing pending payment's expiry to `hours` from now (not
 * added to its old expires_at, since that may already be in the past —
 * this is meant to un-expire an invoice, not extend a still-active one by
 * a fixed offset). Only affects payments still in 'pending' status.
 */
export async function extendPaymentExpiry(env, paymentId, hours) {
    const expiresAt = new Date(Date.now() + hours * 3600000).toISOString().slice(0, 19).replace("T", " ");
    await env.DB.prepare(
        "UPDATE pending_payments SET expires_at=? WHERE id=? AND status='pending'"
    ).bind(expiresAt, paymentId).run();
    return expiresAt;
}

/**
 * Changes a still-pending payment's amount. The WHERE status='pending' guard
 * makes this a no-op if the payment got matched between the caller's check
 * and this call — the caller must re-read the row afterward to confirm the
 * new amount actually took (see functions/api/payment/update-amount.js).
 */
export async function updatePaymentAmount(env, paymentId, newAmount) {
    await env.DB.prepare(
        "UPDATE pending_payments SET amount_rials=? WHERE id=? AND status='pending'"
    ).bind(newAmount, paymentId).run();
    return await getPaymentById(env, paymentId);
}
...
export async function getSmsLogs(env, limit = 300) {
    const { results } = await env.DB.prepare(
        "SELECT * FROM sms_logs ORDER BY received_at DESC LIMIT ?"
    ).bind(limit).all();
    return results;
}

// transactions.sms_log_id references sms_logs(id), so a matched/confirmed
// SMS can't just be deleted outright — unlink it from its transaction first
// (the transaction record itself is preserved, only the back-reference to
// the raw SMS text is cleared) so the delete never fails on a FK violation.

export async function deleteSmsLog(env, id) {
    await env.DB.batch([
        env.DB.prepare("UPDATE transactions SET sms_log_id=NULL WHERE sms_log_id=?").bind(id),
        env.DB.prepare("DELETE FROM sms_logs WHERE id=?").bind(id),
    ]);
}

export async function deleteAllSmsLogs(env) {
    await env.DB.batch([
        env.DB.prepare("UPDATE transactions SET sms_log_id=NULL WHERE sms_log_id IS NOT NULL"),
        env.DB.prepare("DELETE FROM sms_logs"),
    ]);
}

export async function deleteSmsLogsInRange(env, fromStr, toStr) {
    await env.DB.batch([
        env.DB.prepare(
            `UPDATE transactions SET sms_log_id=NULL
             WHERE sms_log_id IN (SELECT id FROM sms_logs WHERE received_at BETWEEN ? AND ?)`
        ).bind(fromStr, toStr),
        env.DB.prepare("DELETE FROM sms_logs WHERE received_at BETWEEN ? AND ?").bind(fromStr, toStr),
    ]);
}
...
// "Listen to all": forward the raw text of every SMS received via
// POST /api/sms/receive to the admin on Telegram, regardless of whether it
// matched/confirmed a payment. Off by default (opt-in, avoids surprising
// notification volume for existing deployments).
export async function isNotifyAllSmsEnabled(env) {
    return (await getSetting(env, "notify_all_sms")) === "1";
}

export async function setNotifyAllSms(env, enabled) {
    await setSetting(env, "notify_all_sms", enabled ? "1" : "0");
}
